...
Adapted from claude-code-hooks-main for macOS + iTerm2.

Modes:
- headless (default): Run via script(1) with PTY
- interactive: Run in tmux session
- auto: Switch to interactive if prompt contains slash commands
...
DEFAULT_CLAUDE = os.environ.get(
    "CLAUDE_CODE_BIN",
    "/Users/dangyao/.nvm/versions/node/v22.22.0/bin/claude",
)
...
def run_with_pty(cmd: list[str], cwd: str | None, env: dict[str, str] | None = None) -> int:
    """Run command with a pseudo-terminal via script(1)."""
    ...
    # macOS script(1) syntax differs from Linux
    if sys.platform == "darwin":
        proc = subprocess.run(
            [script_bin, "-q", "/dev/null", *cmd],
...
def run_interactive_tmux(args: argparse.Namespace) -> int:
    ...
    socket_dir = args.tmux_socket_dir or os.environ.get("TMPDIR", "/tmp")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run Claude Code reliably on macOS (headless or interactive via tmux)")
    ...
    ap.add_argument("--teammate-mode", choices=["auto", "in-process", "tmux"], default=None)
