...
**注意**：`OPENCLAW_HOOK_TOKEN` 对应的是 `hooks.token`，不是 `gateway.auth.token`。如果两者设为相同值则无需区分。
...
所有请求自动携带 `Authorization: Bearer` header（如果 `OPENCLAW_HOOK_TOKEN` 已设置）。
...
- [ ] 环境变量 `OPENCLAW_GATEWAY` + `OPENCLAW_HOOK_TOKEN` 已设置
...
**Gateway 返回 401：**
- 检查 `OPENCLAW_HOOK_TOKEN` 是否与 `openclaw.json` 中 `hooks.token` 一致
